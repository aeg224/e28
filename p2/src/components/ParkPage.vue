...
<center><h2> {{parkData.fullName}}</h2></center>
    ...
    <div class=" col" style="width: 25%;" v-for="image in parkData.images" :key="image.url">
    <a :href="image.url" ><img class="" style="width: 200px; padding-right: 1em;" :src="image.url" alt="Card image cap"></a>
    </div>
</div>
  </div>
<p><h4>Description: </h4> {{parkData.description}}
...
<div class='container mx-5 pt-5'>
  ...
  <h4>Activities</h4>
<ul>
  <li v-for="activity in parkData.activities" :key="activity.id">{{activity.name}}</li>
  </ul>
 </div>
   <div class='col'>
...
<p> <h4> Weather Info:</h4>: {{parkData.weatherInfo}}
<p> <h4> Directions:</h4>: {{parkData.directionsInfo}}
For more information on directions, please visit: {{parkData.directionsUrl}}
  <p>
...
<div class="row">
    ...
    <h3 class="card-title text-dark" style="text-align: left;">Leave A Comment!</h3>

        <form class='mx-1 px-1'>
        <div class="form-group">
            <label for="name">Name</label>
            <input v-model='username' type="text" class="form-control" id="name" placeholder="Enter Name">
            <small v-if="showNameError" id="errorMessage" class="form-text text-danger">{{nameError}}</small>
        </div>
        <div class="form-group">
            <label for="comment">Email address</label>
            <textarea v-model='comment' type="text" class="form-control" id="comment" rows="5" placeholder="Enter Comment"></textarea>
...
<div class="col card text-dark" style="width: 18rem;">
  <ul class="list-group list-group-flush">
    <li class="list-group-item" v-for="comment in parkComments" :key='comment.id'>{{comment.name}}'s comment : {{comment.comment}}</li>
  </ul>
</div>
          ...
          submitComment(){
            ...
            axios.post('comments', {
                ...
                if (!response.data.success){
                  ...
                  this.parkComments.push({name: this.username, comment: this.comment});
               }
            });
          },
          setErrors(error){
